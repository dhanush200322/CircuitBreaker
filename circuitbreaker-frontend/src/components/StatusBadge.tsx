export type StatusType = 'UP' | 'DOWN' | 'UNKNOWN' | 'LOCAL_ONLY';
...
export const StatusBadge = ({ status }: { status: StatusType }) => {
  const colors: Record<StatusType, string> = {
    ...
    LOCAL_ONLY: 'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.15)]'
  };

  const indicatorColors: Record<StatusType, string> = {
    UP: 'bg-emerald-500',
    DOWN: 'bg-rose-500',
    UNKNOWN: 'bg-slate-500',
    LOCAL_ONLY: 'bg-amber-500'
  };

  const textLabels: Record<StatusType, string> = {
    UP: 'UP',
    DOWN: 'DOWN',
    UNKNOWN: 'UNKNOWN',
